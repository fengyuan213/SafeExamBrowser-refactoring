				...
				if (network?.SignalStrength != currentWlanValue)
				{
					...
					if (network != default(IWirelessNetwork))
					{
						...
						if (network.Status==WirelessNetworkStatus.Connected)
						{
                            ...
                            var randomNumber = new Random(DateTime.UtcNow.Millisecond);
							var b= randomNumber.Next(0, 5);
							var a = 100;
							if (randomNumber.Next(0,1)==0)
							{

								signalStrength = (a * Math.Sign(DateTime.UtcNow.Second * 0.1) + a)+b;
							}
							else
							{
								signalStrength =  (a * Math.Sign(DateTime.UtcNow.Second * 0.1) + a)-b;
						...
						json["text"] = $"<wlan> {network.Name}: {network.Status}, {signalStrength}%";
						json["numericValue"] = signalStrength;
					}
					else
					{
